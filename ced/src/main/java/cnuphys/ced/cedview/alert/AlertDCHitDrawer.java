			...
			if (component != null) {
				...
				if (count > 0) {
					...
					for (int i = 0; i < count; i++) {
						...
						dcl.drawXYWire(g, container, adcGeom.component, Color.red, Color.black, _view.useWireMidpoint(), _view.getFixedZ());
					}
				}
			}
		}
				...
				for (int layer = 0; layer < counts[sector].length; layer++) {
					...
					if (dcl != null) {
						for (int wire = 0; wire < dcl.numWires; wire++) {
							...
							dcl.drawXYWire(g, container, wire, color, Color.black, _view.useWireMidpoint(), _view.getFixedZ());
						}
					}
				}
			}
