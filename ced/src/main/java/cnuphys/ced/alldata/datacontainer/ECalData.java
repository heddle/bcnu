...
public class ECalData implements IDataContainer {
	
	public static final int NOPID = -999999;


	// the singleton
	private static volatile ECalData _instance;
	...
	/** 0-based planes (0, 1) for (inner, outer) for REC::Calorimeter */
	public ArrayList<Byte> recPlane = new ArrayList<>();
	...
	ArrayList[] lists = {sector, plane, view, strip, adc, tdc,
            recSector, recPlane, recView, recTime, recEnergy, recX, recY, recZ, recPIndex};
	 ...
	 */
	public static ECalData getInstance() {
		if (_instance == null) {
			synchronized (ECalData.class) {
				if (_instance == null) {
					_instance = new ECalData();
				}
			}
		}
		return _instance;
	...
	// update the raw data
	private void updateRaw(DataEvent event) {
