	...
	public NoiseParameterDialog() {
		...
		for (int superLayer = 0; superLayer < 6; superLayer++) {
			...
			_clonedParameters[superLayer]
					.copyEditableParameters(SNRManager.getInstance().getParameters(0, superLayer));
		}
	...
	private void doClose(int reason) {
		...
		if ((reason == DialogUtilities.OK_RESPONSE) || (reason == DialogUtilities.APPLY_RESPONSE)) {
			...
			for (int sector = 0; sector < 6; sector++) {
				for (int superLayer = 0; superLayer < 6; superLayer++) {
					SNRManager.getInstance().getParameters(sector, superLayer)
